module ApplicationHelper
  ...
  def insert_extras(body)
    body = body.gsub(/\<p\>\[notes\:(.+)\]/) do |tagname|
      ...
      output += '</table>'
      output += '<a href="/post?tags=activity:#{tagname},#{tagname},seeks:replications&title=How%20to%20do%20X" class="btn btn-primary add-activity">Add an activity</a> <a href="/post?tags=#{tagname},question:#{tagname},request:activity&template=question&title=How%20do%20I...&redirect=question" class="btn btn-primary request-activity">Request an activity guide</a>'
      output += '_Activities should include a materials list, costs and a step-by-step guide to construction with photos. Learn what [makes a good activity here](/notes/warren/09-17-2016/what-makes-a-good-activity)._'      

class SearchService
  ...
  def tagNearbyNodes(coordinates, tag, period = { "from" => nil, "to" => nil }, sort_by = nil, order_direction = nil, limit = 10)
    ...
    raise("If 'from' is not null, must contain date") if !(period["from"].nil?) && !(period["from"].is_a? Date)
    raise("If 'to' is not null, must contain date") if !(period["to"].nil?) && !(period["to"].is_a? Date)

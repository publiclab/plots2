...
class ApplicationController < ActionController::Base
  ...
  def alert_and_redirect_moderated
    if @node.author.status == 0 && !(current_user && (current_user.role == 'admin' || current_user.role == 'moderator'))
      ...
      flash[:warning] = "This is a Draft note. Kindly complete it and publish it using <a class='btn btn-success' href='/notes/publish_draft/#{@node.id}'>Publish Draft</a> button. You can also share this draft using <a class='btn btn-success' onclick='get_secret_URL()'>Share private link</a> "

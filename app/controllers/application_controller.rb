...
class ApplicationController < ActionController::Base
  ...
  def alert_and_redirect_moderated
    ...
    elsif @node.status == 3 && (current_user && (current_user.id == @node.author.id || @node.has_tag("with:#{current_user.username}") || current_user.role == 'admin' || current_user.role == 'moderator')) && !flash[:first_time_post]

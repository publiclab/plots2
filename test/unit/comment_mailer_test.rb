...
class CommentMailerTest < ActionMailer::TestCase
  ...
  test "notify note author" do
    ...
    assert_equal [user.email], email.to
    assert_equal "New comment on '" + comment.parent.title + "'", email.subject
    assert email.body.include?("Hi! There's been a comment to your question '<a href='https://publiclab.org#{comment.parent.path(:question)}'>#{comment.parent.title}</a>'")
  end
  ...
  test "notify answer author" do
    ...
    assert email.body.include?("Hi! There's been a new comment to your answer on '<a href='https://publiclab.org#{comment.parent.path(:question)}#a#{comment.answer.id}'>#{comment.parent.title}</a>'")
  end
end

...
class AdminMailerTest < ActionMailer::TestCase
  ...
  test "notify_node_moderators" do
    node1 = node(:one)
    moderators = User.where(role: ['moderator', 'admin'])
    assert moderators.length > 0

    assert_difference 'ActionMailer::Base.deliveries.size', 1 do
      # send the email
      AdminMailer.notify_node_moderators(node1)
    end
    ...
    assert_equal "[New Public Lab poster needs moderation] " + node1.title, email.subject
    assert email.body.include?("First-time poster <a href='https://#{request_host}/profile/#{node1.author.name}'>#{node1.author.name}</a> has submitted their first research note!")
    assert email.body.include?(node1.latest.render_body_email(request_host))
  end
  ...
  test "notify_author_of_approval" do
    ...
    assert_difference 'ActionMailer::Base.deliveries.size', 1 do
      # send the email
      AdminMailer.notify_author_of_approval(node1, moderator)
    end
    ...
    email = ActionMailer::Base.deliveries.last
    assert_not_nil email.to
    assert_equal [node1.author.mail], email.to
  ...
  test "notify_moderators_of_approval" do
    ...
    assert_difference 'ActionMailer::Base.deliveries.size', 1 do
      # send the email
      AdminMailer.notify_moderators_of_approval(node1, moderator)
    end
    ...
    assert_equal "[New Public Lab poster needs moderation] " + node1.title, email.subject
    time_ago = time_ago_in_words(node1.created_at)
  ...
  test "notify_moderators_of_spam" do
    ...
    assert_difference 'ActionMailer::Base.deliveries.size', 1 do
      # send the email
      AdminMailer.notify_moderators_of_spam(node1, moderator)
    end

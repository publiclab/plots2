...
class NodeTest < ActiveSupport::TestCase
  ...
  test 'node mysql native fulltext search returning two tag-based matches' do
    assert Node.count > 0
    if ActiveRecord::Base.connection.adapter_name == 'Mysql2'
      nodes = Node.search(query: 'awesome cool', limit: 1000)
      assert_not_nil nodes
      assert_equal 1, nodes.length
      assert_equal nodes(:one), nodes.first
    end
  end
  ...
  test 'should have tags, node_tags, and tagnames' do
    ...
    assert_equal 'test awesome cool spectrometer activity:spectrometer', node.tagnames.join(' ')
    # used to generate CSS classes:
    assert_equal 'tag-test tag-awesome tag-cool tag-spectrometer tag-activity-spectrometer', node.tagnames_as_classes
  end

...
class SubscriptionControllerTest < ActionController::TestCase
  ...
  test "should redirect properly when subscribing to multiple tags" do
   ...
   test 'sorting of digest by title' do
    UserSession.create((users(:bob)))
    get :digest
    unsort = assigns(:wikis)
    get :digest, params: { sort: "title"}
    wikis = assigns(:wikis)
    sorted_wikis = unsort.sort_by { |wiki| wiki.title.downcase }
    assert_equal wikis, sorted_wikis
  end  
  ...
  test 'sorting of digest by likes' do
   ...
   wikis = assigns(:wikis)
   sorted_wikis =  unsort.sort_by(&:cached_likes).reverse!
   assert_equal wikis, sorted_wikis
  end
  ...
  test 'sorting of digest by views' do 
    ...
    wikis = assigns(:wikis)
    sorted_wikis = unsort.sort_by(&:views).reverse!
    assert_equal wikis, sorted_wikis
  end
  ...
  test 'sorting of digest by edits' do
    ...
    wikis = assigns(:wikis)
    sorted_wikis = unsort.sort_by(&:drupal_node_revisions_count).reverse!
    assert_equal wikis, sorted_wikis
  end
  ...
  test 'sorting by last edited' do
    ...
    wikis = assigns(:wikis)
    sorted_wikis = unsort.sort_by(&:changed).reverse!
    assert_equal wikis, sorted_wikis
  end
end

...
class TagControllerTest < ActionController::TestCase
  ...
  test "adds comment when awarding a barnstar" do
    ...
    assert_difference 'DrupalComment.count' do
      ...
      assert_equal "#{User.first.username} awards a <a href=\"//#{Rails.root}/wiki/barnstars\">barnstar</a> to #{DrupalNode.last.drupal_users.name} for their awesome contribution!", DrupalComment.last.body
    end
  end

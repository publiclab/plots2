...
Our mutual aims of democratizing inexpensive and accessible do-it-yourself techniques have allowed us to create a collaborative network of practitioners who actively re-imagine the human relationship with the environment. Our goals are supported and facilitated by a system that questions and even challenges how collaborative work can happen.
...
7. You could choose to use mysql2 or sqlite3 as your database
   _ If mysql2, run `cp config/database.yml.mysql.example config/database.yml` to make a copy of `config/database.yml.mysql.example` in `config/database.yml`
   _ If sqlite3, run `cp config/database.yml.sqlite.example config/database.yml` to make a copy of `config/database.yml.sqlite.example` in `config/database.yml`.
...
4. [Click here](https://github.com/javan/whenever) to know about how to write cron jobs.

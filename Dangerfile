message "@#{github.pr_author} Thank you for your pull request! I'm here to help with some tips and recommendations. Please take a look at the list above and help us review and accept your contribution!"

warn "Your pull request is on the `master` branch. Please [make a separate feature branch](https://publiclab.org/wiki/contributing-to-public-lab-software#A+sample+git+workflow)) with a descriptive name like `new-blog-design` while making PRs in the future." if github.branch_for_head == 'master'

unless git.commits.any? { |c| c.message =~ /#[\d]+/ }
  warn "This pull request doesn't link to a issue number. Please refer to the issue it fixes (if any) in the format: `Fixes #123`."
end

warn "You have added multiple commits. It's helpful to [squash them](http://gitready.com/advanced/2009/02/10/squashing-commits-with-rebase.html) if the individual changes are small." if git.commits.size > 1
...
if github.pr_body.include?("* [ ]") && !github.pr_title.include?("[WIP]")
  warn "It looks like you haven't marked all the checkboxes. Help us review and accept your suggested changes by going through the steps one by one. If it is still a 'Work in progresss', please include '[WIP]' in the title."
end
